...
class ElearningModule_model
{
  ...
  public function countUserAkses()
  {
    $this->db->query('SELECT elearningModule.*, COUNT(DISTINCT userModuleAkses.userNik) AS totalUser
                      FROM elearningModule
                      LEFT JOIN userModuleAkses ON elearningModule.elearningModuleId = userModuleAkses.moduleId
                      WHERE elearningModule.elearningCourseId = 19
                      GROUP BY elearningModule.elearningModuleId');
    return $this->db->resultSet();
  }
}
